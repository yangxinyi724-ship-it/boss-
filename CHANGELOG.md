...
- **ZhilianClient 内部 HTTP 客户端骨架**（Issue #140 Week 2 起步）— 新增 `src/pet_boss/api/zhilian_client.py`：
...
- `src/pet_boss/platforms/` 新子包定义 Platform ABC + 注册表 + BossPlatform adapter
...
- `Platform` / `BossPlatform` / `ZhilianPlatform` / `get_platform` / `list_platforms` 导出到 `from pet_boss import ...` 公共 API 面
...
- `src/pet_boss/platforms/zhilian.py` — 智联招聘 stub 实现
...
- `chat_snapshot` 使用 `pet_boss.output.Logger` 替代裸 `logger` 参数
...
### Added
- **包级公开接口**（#94）— `pet_boss.__all__` 导出 14 个核心符号（AuthManager / BossClient / CacheStore / JobItem / JobDetail / AIService / ResumeData 等），下游项目可直接 `from pet_boss import X` 使用，不再需要深路径 import
...
### Changed
- `src/pet_boss/__init__.py` 补完整 docstring 说明包的公开 API 契约
