...
def test_close_browser_session_keeps_httpx_client():
	"""close_browser_session 仅关闭浏览器，不标记整个 client 为 closed。"""
	client = BossClient(_StubAuth())
	fake_browser = MagicMock()
	client._browser_session = fake_browser
...
@patch("pet_boss.api.client.atexit")
def test_atexit_safeguard_closes_open_clients(mock_atexit):
	"""atexit handler 应关闭还未被 close 的 BossClient。"""
	from pet_boss.api.client import _close_open_clients, _OPEN_CLIENTS
