...
from pet_boss.api.browser_client import (
	CDP_DEFAULT_URL,
	HOME_URL,
	_HEADLESS_NETWORKIDLE_GRACE_MS,
	_NAV_TIMEOUT_MS,
	BrowserSession,
	is_browser_session_lost_message,
)
...
def test_read_devtools_active_port_missing(tmp_path):
	with patch("pet_boss.api.browser_client._CHROME_USER_DATA_CANDIDATES", [tmp_path / "nonexistent"]):
...
def test_read_devtools_active_port_found(tmp_path):
	...
	with patch("pet_boss.api.browser_client._CHROME_USER_DATA_CANDIDATES", [tmp_path]):
		ws = BrowserSession._read_devtools_active_port()
...
def test_refresh_home_for_new_round():
	session = BrowserSession(cookies={"wt2": "abc"}, user_agent="UA")
	session._started = True
	mock_page = MagicMock()
	session._page = mock_page

	session.refresh_home_for_new_round()
...
def test_ensure_started_launches_visible_browser_immediately():
	session = BrowserSession(cookies={"wt2": "abc"}, user_agent="UA")
	mock_pw = MagicMock()
	sentinel = {"started": False}
	...
	with (
		patch("pet_boss.api.browser_client.sync_playwright") as mock_sync_playwright,
		patch.object(session, "_start_headless", side_effect=mark_started) as mock_start_headless,
	):
		mock_sync_playwright.return_value.start.return_value = mock_pw
		session._ensure_started()

	assert sentinel["started"] is True
...
def test_is_browser_session_lost_message_detects_target_closed():
	assert is_browser_session_lost_message("Target page, context or browser has been closed")
	assert not is_browser_session_lost_message("network timeout")


def test_restart_session_returns_structured_outcome_for_headless():
	session = BrowserSession(cookies={}, user_agent="ua")
	session._is_cdp = False
	session._pw = MagicMock()
	session._browser = MagicMock()
	session._context = MagicMock()
	...
	with patch.object(session, "_teardown_headless") as teardown:
		with patch.object(session, "_start_headless") as start:
			outcome = session.restart_session()

	teardown.assert_called_once_with(keep_playwright=True)
	start.assert_called_once()
	assert outcome["close_ok"] is True
	assert outcome["launch_ok"] is True
	assert outcome["ok"] is True
	assert outcome["sequence"] == "stall_then_close"


def test_request_marks_browser_lost_when_evaluate_raises_target_closed():
	session = BrowserSession(cookies={}, user_agent="ua")
	session._started = True
	session._page = MagicMock()
	session._page.evaluate.side_effect = RuntimeError("Target page, context or browser has been closed")
	session._throttle = MagicMock()

	result = session.request("GET", "https://example.com/api")

	assert result["browser_lost"] is True
	assert result["code"] == -2


def test_focus_window_brings_automation_page_to_front():
	session = BrowserSession(cookies={}, user_agent="ua")
	session._started = True
	session._page = MagicMock()
	session._is_cdp = False

	ok = session.focus_window(url="https://www.zhipin.com/web/geek/job")

	assert ok is True
	session._page.goto.assert_called_once()
	session._page.bring_to_front.assert_called_once()
