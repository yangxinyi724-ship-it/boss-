...
from pet_boss.auth.browser import (
	HOME_URL,
	LOGIN_PAGE_URL,
	_NAV_TIMEOUT_MS,
...
@patch("pet_boss.auth.browser.probe_cdp", return_value="ws://localhost/devtools/browser")
@patch("pet_boss.auth.browser.time.sleep", return_value=None)
def test_login_via_cdp_stops_playwright_on_timeout(mock_sleep, mock_probe_cdp):
	...
	with patch("pet_boss.auth.browser.sync_playwright", return_value=mock_launcher):
...
@patch("pet_boss.auth.browser._extract_stoken", return_value="fresh-stoken")
@patch("pet_boss.auth.browser.time.sleep", return_value=None)
def test_login_via_browser_tolerates_networkidle_timeout(mock_sleep, mock_extract_stoken):
	...
	with patch("pet_boss.auth.browser.sync_playwright", return_value=_mock_playwright_context(mock_browser)):
