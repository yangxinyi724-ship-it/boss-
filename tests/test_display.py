...
class TestHandleOutput:
	def test_json_mode_emits_json(self):
		ctx = MagicMock()
		ctx.obj = {"json_output": True}
		with patch("pet_boss.display.emit_success") as mock_emit:
...
class TestHandleErrorOutput:
	def test_json_mode_emits_error(self):
		ctx = MagicMock()
		ctx.obj = {"json_output": True}
		with patch("pet_boss.output.emit_error") as mock_emit:
...
class TestHandleAuthErrors:
	def test_auth_required(self):
		from pet_boss.auth.manager import AuthRequired
		ctx = MagicMock()
		ctx.obj = {"json_output": True}
		...
		with patch("pet_boss.display.handle_error_output") as mock_err:
			impl(ctx)
	...
	def test_token_refresh_failed(self):
		from pet_boss.auth.manager import TokenRefreshFailed
		ctx = MagicMock()
		ctx.obj = {"json_output": True}
...
class TestHandleOutputFallback:
	def test_tty_mode_without_render_emits_json(self):
		...
		ctx.obj = {"json_output": False}
		with patch.object(sys, "stdout") as mock_out, \
			patch("pet_boss.display.emit_success") as mock_emit:
			mock_out.isatty.return_value = True
			handle_output(ctx, "test", {"key": "val"})
			mock_emit.assert_called_once()
...
class TestHandleAuthErrorsAccountRisk:
	def test_account_risk_non_cdp_stops_automation(self):
		from pet_boss.api.client import AccountRiskError
		ctx = MagicMock()
		ctx.obj = {"json_output": True}
...
class TestRenderers:
	def test_render_job_table_empty(self):
		from pet_boss.display import render_job_table
	...
	def test_render_job_detail_minimal(self):
		from pet_boss.display import render_job_detail
	...
	def test_render_status_logged_in(self):
		from pet_boss.display import render_status
	...
	def test_render_simple_list_empty(self):
		from pet_boss.display import render_simple_list
	...
	def test_render_sectioned_record_mixed(self):
		from pet_boss.display import render_sectioned_record
	...
	def test_render_string_grid_empty(self):
		from pet_boss.display import render_string_grid
	...
	def test_render_export_summary_with_path(self):
		from pet_boss.display import render_export_summary
