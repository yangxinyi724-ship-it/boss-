...
def test_machine_id_written_to_auth_dir_and_reused(tmp_path, monkeypatch):
	monkeypatch.delenv("BOSS_AGENT_MACHINE_ID", raising=False)
	store = _make_store(tmp_path)
	with patch.object(store, "_detect_machine_id", return_value="detected-once"):
		first = store._get_machine_id()
		second = store._get_machine_id()
	assert first == second == "detected-once"
	assert (tmp_path / "auth" / "machine.id").read_text(encoding="utf-8") == "detected-once"
...
def test_machine_id_linux_reads_machine_id_file(tmp_path, monkeypatch):
	...
	with patch("pet_boss.auth.token_store.platform.system", return_value="Linux"), \
		 patch("pet_boss.auth.token_store.Path") as mock_path_cls:
...
def test_machine_id_windows_reg_query(tmp_path, monkeypatch):
	...
	with patch("pet_boss.auth.token_store.platform.system", return_value="Windows"), \
		 patch("pet_boss.auth.token_store.shutil.which", return_value="/windows/reg.exe"), \
		 patch("pet_boss.auth.token_store.subprocess.run", return_value=fake_result):
		result = store._get_machine_id()
...
def test_machine_id_unknown_system_uses_fingerprint_fallback(tmp_path, monkeypatch):
	...
	with patch("pet_boss.auth.token_store.platform.system", return_value="UnknownOS"):
		result = store._get_machine_id()
		# 应返回 64 位 sha256 hex
		assert len(result) == 64
...
def test_machine_id_oserror_exception_falls_back(tmp_path, monkeypatch):
	...
	with patch("pet_boss.auth.token_store.platform.system", return_value="Darwin"), \
		 patch("pet_boss.auth.token_store.shutil.which", return_value="/usr/sbin/ioreg"), \
		 patch("pet_boss.auth.token_store.subprocess.run", side_effect=OSError("denied")):
		result = store._get_machine_id()
...
def test_refresh_lock_timeout_breaks_stale_lock(tmp_path, monkeypatch):
	...
	import pet_boss.auth.token_store as ts
