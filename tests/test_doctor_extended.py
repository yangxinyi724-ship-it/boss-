...
def _base_patches():
	...
	return {
		"auth": "pet_boss.commands.doctor.AuthManager",
		"cdp": "pet_boss.commands.doctor.probe_cdp",
		"httpx": "pet_boss.commands.doctor.httpx.get",
		"cookie": "pet_boss.commands.doctor.extract_cookies",
	}
...
def test_all_checks_pass(tmp_path):
	...
	env_dependent = {"patchright", "patchright_chromium", "browser", "auth_salt", "recruiter_read_health"}
...
@patch("pet_boss.commands.doctor.shutil.which", return_value=None)
...
@patch("pet_boss.commands.doctor.get_platform_instance")
def test_doctor_live_probe_adds_readonly_probe_checks(mock_platform_cls, tmp_path):
	paths = _base_patches()
	runner = CliRunner()
	with (
		patch(paths["auth"]) as mock_auth,
