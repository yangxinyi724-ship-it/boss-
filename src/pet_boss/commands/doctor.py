...
from pet_boss.auth.browser import probe_cdp, _DEFAULT_CDP_URL
from pet_boss.auth.cookie_extract import extract_cookies
from pet_boss.auth.health import assess_auth_health, auth_config_for_platform
from pet_boss.auth.manager import AuthManager
from pet_boss.commands._platform import get_platform_instance
from pet_boss.display import handle_output, render_simple_list
...
def doctor_cmd(ctx: click.Context, live_probe: bool) -> None:
	...
	if cdp_ok:
		add_check(
			"browser_channel",
			"ok",
			"CDP 兼容通道可用；默认低风险模式下不得用于规避平台风控",
		)
	else:
		add_check(
			"browser_channel",
			"warn",
			"CDP 不可用；默认低风险模式不依赖它执行敏感操作",
