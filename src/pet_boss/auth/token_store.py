...
class TokenStore:
	...
	def _get_machine_id(self) -> str:
		...
		machine_id_path = self._auth_dir / "machine.id"
		if machine_id_path.exists():
			stored = machine_id_path.read_text(encoding="utf-8").strip()
			if stored:
				return stored

		machine_id = self._detect_machine_id()
		try:
	...
	def _detect_machine_id(self) -> str:
