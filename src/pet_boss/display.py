...
def handle_error_output(
	...
	*,
	...
	from pet_boss.output import emit_error
...
def render_job_table(
	...
	*,
	...
	show_score = any(
		job.get("profile_score") is not None or job.get("match_score") is not None
		for job in items
	)
	if show_score:
		table.add_column("score", style="magenta", max_width=6)
	...
	for i, job in enumerate(items, 1):
		...
		if show_score:
			score = job.get("profile_score", job.get("match_score", "-"))
...
def handle_auth_errors(command_name: str) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
	...
	def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
		...
		def wrapper(ctx: Any, *args: Any, **kwargs: Any) -> Any:
			...
			from pet_boss.auth.manager import AuthRequired, TokenRefreshFailed
			try:
