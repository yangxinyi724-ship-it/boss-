...
Web：宠物页办公室场景**下方**有「工程洞察」区块（下滚可见），展示观测汇总 / 评测 / **RAG 决策对比**；接口 `GET /api/boss/insights`、`POST /api/boss/eval`、`POST /api/boss/eval/capture`、`POST /api/boss/rag-ablation`。策略/换词决策仍写入 `decisions.jsonl`（不在页内列表展示）。
