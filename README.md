...
- `🧠 用户画像智能系统`：解析简历 → 聊天式访谈 → 职业方向推理 → 自适应岗位评分 → 行为反馈学习。命令：`profile parse` `profile interview` `profile infer` `search --profile-score`
...
- `🔌 多平台抽象`：`Platform` / `RecruiterPlatform` 双注册表已落地；默认低风险模式优先暴露只读和本地辅助链路。命令：`--platform zhipin`
...
```bash
# 指定平台（当前仅 zhipin）
boss --platform zhipin search "Python"
```
...
```python
from pet_boss import AuthManager, BossClient, AuthRequired
...
公开 API（详见 `pet_boss.__all__`）：`AuthManager` / `BossClient` / `CacheStore` / `JobItem` / `JobDetail` / `AIService` / `ResumeData` 等核心类型。
...
| `boss web` / `boss profile web` | **一体化 Web**：画像 + BOSS 直聘搜岗 + 自适应评分 |
| `boss profile upload-pdf <file.pdf>` | CLI 上传 PDF 简历并由 AI 解析 |
| `boss profile run` | **一键启动**：解析 → AI 访谈 → 职业推理（终端内交互） |
| `boss profile parse <resume>` | Step 1：解析简历，提取技能/项目/行业等结构化画像 |
| `boss profile interview start/answer/finish` | Step 2：5~15 题动态访谈，收集职业偏好 |
| `boss profile infer` | Step 3：推理最适合的职业方向与发展路径 |
| `boss profile show` | 查看完整用户画像与学习权重 |
| `boss search "Golang" --profile-score` | Step 4：搜索时按画像自适应评分（9 维加权） |
| `boss profile score <#>` | 对 index_cache 中某条职位做详细匹配分析 |
| `boss profile feedback --action shortlisted` | Step 5：记录行为反馈，优化评分模型 |
| `boss profile reset` | 清除画像数据（保留反馈历史） |
...
# Browser Bridge 诊断
python -m pet_boss.bridge.daemon --serve
